...
if (!isInsideDocker) {
  ...
  if (nextDatabaseUrl !== env.DATABASE_URL) {
    console.log("[start:docker] Running outside Docker: DATABASE_URL host adjusted db -> localhost");
    env.DATABASE_URL = nextDatabaseUrl;
  }

  if (nextS3Endpoint !== env.S3_ENDPOINT) {
    console.log("[start:docker] Running outside Docker: S3_ENDPOINT host adjusted minio -> localhost");
    env.S3_ENDPOINT = nextS3Endpoint;
  }
}

runCommand("npm", ["run", "prisma:generate"], env);
runCommand("npm", ["run", "prisma:deploy"], env);
runCommand("npm", ["run", "db:seed"], env);
runCommand("npm", ["run", "start"], env);
